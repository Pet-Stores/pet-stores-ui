# Histórico de Conversas e Decisões

Este arquivo serve como um índice para todas as sessões e tarefas registradas, divididas entre **Features** (o que foi feito) e **Sessões** (resumo das conversas).

## 🚀 Features (Implementações Técnicas)

| Data | Registro | Temas | Arquivo |
| :--- | :--- | :--- | :--- |
| 2026-05-19 | Tarefa 01 | Build, Dependências, Budgets | [features/01-resolucao-erros-build.md](./features/01-resolucao-erros-build.md) |
| 2026-05-19 | Tarefa 02 | Estratégia MVP, Capacitor, Contexto | [features/02-definicao-estrategia-e-contexto.md](./features/02-definicao-estrategia-e-contexto.md) |
| 2026-05-20 | Tarefa 03 | Design System e Efeitos Premium | [features/03-implementacao-design-system.md](./features/03-implementacao-design-system.md) |

## 💬 Sessões (Resumos de Conversa)

| Data | Registro | Principais Temas | Arquivo |
| :--- | :--- | :--- | :--- |
| 2026-05-19 | Sessão 01 | Síntese: MVP, Arquitetura, Comando Save | [sessions/2026-05-19-sessao-01.md](./sessions/2026-05-19-sessao-01.md) |
| 2026-05-20 | Sessão 01 | Recuperação de Contexto e Protocolo Save | [sessions/2026-05-20-sessao-01.md](./sessions/2026-05-20-sessao-01.md) |
| 2026-05-20 | Sessão 02 | Estratégia de Admin e Controle de Acesso | [sessions/2026-05-20-sessao-02.md](./sessions/2026-05-20-sessao-02.md) |
